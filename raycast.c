...
/*   Updated: 2023/08/30 09:51:43 by onaciri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	horzintal_inter(t_img *img)
{
	...
	yinter = (int)(img->y / (64 * 0.2)) * 64 * 0.2;
	...
	if (sin(img->ray->ray_angel)> 0)//facing up
		ystep *= -1;
	xstep = (64 * 0.2) / tan(img->ray->ray_angel);
...
void	vertical_inter(t_img *img)
{
	...
	xinter = ((int)(img->x / (64 * 0.2)) * (64 * 0.2));
	printf("===%f\n", img->ray->ray_angel);
	...
	if (cos(img->ray->ray_angel) < 0)//facing left
		xstep *= -1;
	ystep = (64 * 0.2) * tan(img->ray->ray_angel);
